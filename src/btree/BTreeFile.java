...
public class BTreeFile extends IndexFile implements GlobalConst {
...

	{
		...
		if(getHeaderPage().get_rootId().pid==INVALID_PAGE) {
			//System.out.println("Enterd into if of root");
			...
			//System.out.println("Enterd into else of insert block");
			//KeyDataEntry newRootEntry = new KeyDataEntry(key,newRootPageId);
			KeyDataEntry newRootEntry;
			newRootEntry = _insert(key,rid,getHeaderPage().get_rootId());
...

	{
		...
		//System.out.println("The page id passed is" + currentPageId);
		...
		}else
		{
			...
			{ 
				try
				{
				newIndexPage.insertRecord(tmpEntry);
				//currentIndexPage=deleteSortedRecord(delRID);
				currentIndexPage.deleteSortedRecord(delRID);
				}catch(Exception e) { }				
			}
			//Spit equal using a for loop
			...
			newIndexPage.setPrevPage(headerPage.get_rootId()); // Check It Later
			newIndexPage.deleteSortedRecord(delRID);//Check Later
		...
		{ 
			//System.out.println("Entered Leaf Block in _insert");
			BTLeafPage currentLeafPage = new BTLeafPage(currentPageId,AttrType.attrInteger); //changed
			//BTLeafPage currentLeafPage;
			PageId currentLeafPageId= currentLeafPage.getCurPage();
			//PageId currentLeafPageId;
			//PageId nextPageId= currentLeafPage.getPageNoByKey(key);
			//upEntry=_insert(key,rid,currentLeafPageId);//Error might be
			//upEntry=new KeyDataEntry(key,currentLeafPageId);
			...
			{ 
				//System.out.println("*********Entered the Storage Out Else Block");
				...
				{ 
					try
					{//System.out.println("In Frist For loop"+ " The Key Is "+ tmpEntry.key+" "+delRID);
					newLeafPage.insertRecord(tmpEntry.key,((LeafData)tmpEntry.data).getData());
					}catch(Exception e) { System.out.println("There is exception"+ e );}
					//currentIndexPage=deleteSortedRecord(delRID);
					boolean x=currentLeafPage.deleteSortedRecord(delRID);
					//System.out.println("The bool is "+x);
									
				}
				//System.out.println(newLeafPage.available_space());
				//Split Even for loop
				KeyDataEntry tmpEntry1;
				RID delRID1=new RID();
				KeyDataEntry tmpEntry2;
				
				for(tmpEntry1=newLeafPage.getFirst(delRID1);tmpEntry1!=null;tmpEntry1=newLeafPage.getFirst(delRID1))
						{   int n = BT.getKeyDataLength(key, NodeType.LEAF);
							//System.out.println("Enterd second for loop*******"+ newLeafPage.available_space());
							
							/*if(currentLeafPage.available_space()>(2*n))
							{ 	System.out.println("****IN THE IF*********");
								try
								{
									System.out.println("The key is "+tmpEntry1.key);
								currentLeafPage.insertRecord(tmpEntry1.key,((LeafData)tmpEntry1.data).getData());
						...
						currentLeafPage.insertRecord(tmpEntry1.key,((LeafData)tmpEntry1.data).getData());
						newLeafPage.deleteSortedRecord(delRID1);
						
						}
				
				
				tmpEntry2 = newLeafPage.getFirst(delRID);
				if(BT.keyCompare(key, tmpEntry2.key)>0)
				{ 
					//newLeafPage.insertKey(upEntry.key, ((IndexData)upEntry.data).getData());
					
					//newLeafPage.insertRecord(tmpEntry2.key,((LeafData)tmpEntry2.data).getData());
				...
				{ 
					...
					//currentLeafPage.insertRecord(tmpEntry2.key,((LeafData)tmpEntry2.data).getData());
				...
				upEntry=newLeafPage.getFirst(delRID);
				//newIndexPage.setPrevPage(headerPage.get_rootId()); // Check It Later
				//newIndexPage.deleteSortedRecord(delRID);//Check Later
				...
				tmpEntry2 = newLeafPage.getFirst(delRID);
				//globalid=newLeafPageId;
				KeyDataEntry upEntry3=new KeyDataEntry(tmpEntry2.key,newLeafPage.getCurPage());
				//unpinPage(currentLeafPage.getCurPage());
